        ...
        let res = match c.frame(spf) {
            Ok(frame) => {
                ...
                if frame.valid() {
                    // 新增：4K缩放到1080p
                    let mut frame = frame;
                    let (src_width, src_height) = match &frame {
                        scrap::Frame::PixelBuffer(pb) => (pb.width(), pb.height()),
                        scrap::Frame::Texture(_) => (capture_width, capture_height),
                    };
                    let (target_width, target_height) = (1920, 1080);
                    let need_resize = src_width > target_width || src_height > target_height;
                    if need_resize {
                        if let scrap::Frame::PixelBuffer(pb) = &frame {
                            // 只处理BGRA格式
                            if pb.pixfmt() == scrap::Pixfmt::BGRA {
                                // 使用 image crate 进行缩放
                                let stride = pb.stride()[0];
                                let raw = pb.data();
                                // 构造 image::ImageBuffer
                                if let Ok(img) = image::ImageBuffer::<image::Bgra<u8>, _>::from_raw(src_width as u32, src_height as u32, raw) {
                                    let img = image::DynamicImage::ImageBgra8(img);
                                    let resized = img.resize(target_width as u32, target_height as u32, image::imageops::FilterType::Triangle);
                                    // 转回BGRA
                                    let resized_bgra = resized.to_bgra8();
                                    // 构造 PixelBuffer
                                    let resized_pb = scrap::PixelBuffer::new(resized_bgra.as_raw(), scrap::Pixfmt::BGRA, target_width, target_height);
                                    // 用缩放后的 PixelBuffer 替换
                                    frame = scrap::Frame::PixelBuffer(resized_pb);
                                }
                            }
                        }
                    }
